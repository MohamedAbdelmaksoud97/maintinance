create or replace function public.area_ignores_shutdown_windows(area_code text)
returns boolean
language sql
immutable
security invoker
as $$
  select coalesce(area_code, '') in ('FINISH_MILL', 'PACKHOUSE');
$$;

grant execute on function public.area_ignores_shutdown_windows(text) to authenticated;
...
declare
  ...
  ignores_shutdown_windows boolean;
...
begin
  ...
  loop
    ...
    ignores_shutdown_windows := public.area_ignores_shutdown_windows(point_row.area_code);
    v_schedule_line_code := case when ignores_shutdown_windows then null else v_line_code end;
    ...
    set data_quality_status = case
          when v_line_code is null and not ignores_shutdown_windows then 'MISSING_DATA'
          else 'COMPLETE'
        end,
        needs_data_review = v_line_code is null and not ignores_shutdown_windows,
        ...
        original_values = coalesce(original_values, '{}'::jsonb) || jsonb_build_object(
          ...
          'schedule_line_code', v_schedule_line_code,
          'ignores_shutdown_windows', ignores_shutdown_windows,
          'step_days', step_days,
    ...
    due_date := case
      when uses_operating_days then public.add_operating_days(anchor_date, step_days, v_schedule_line_code)
      else anchor_date + step_days
    end;

    while due_date < target_start loop
      due_date := case
        when uses_operating_days then public.add_operating_days(due_date, step_days, v_schedule_line_code)
    ...
    while due_date <= horizon loop
      previous_raw_due_date := case
        when uses_operating_days then public.subtract_operating_days(due_date, step_days, v_schedule_line_code)
        else due_date - step_days
      end;
      previous_scheduled_date := case
        when uses_operating_days then previous_raw_due_date
        when ignores_shutdown_windows then previous_raw_due_date
        else public.adjust_maintenance_due_date(previous_raw_due_date, v_schedule_line_code, point_row.execution_condition)
      end;
      v_scheduled_date := case
        when uses_operating_days then due_date
        when ignores_shutdown_windows then due_date
        else public.adjust_maintenance_due_date(due_date, v_schedule_line_code, point_row.execution_condition)
      end;
      needs_shutdown := case
        when ignores_shutdown_windows then false
        else point_row.execution_condition = 'shutdown'
          and not exists (
            select 1
            from public.shutdown_windows sw
            where sw.is_active = true
              and sw.line_code = v_schedule_line_code
      ...
      if uses_operating_days and v_schedule_line_code is not null and due_date <> previous_raw_due_date + step_days then
...
update public.planned_tasks pt
set status_id = old_status.id,
    updated_at = now(),
    original_values = coalesce(pt.original_values, '{}'::jsonb) || '{"replaced_by_continuous_area_rule": true}'::jsonb
...
where pt.equipment_id = e.id
  and public.area_ignores_shutdown_windows(a.code)
  and coalesce(pt.original_values->>'ignores_shutdown_windows', 'false') <> 'true'
